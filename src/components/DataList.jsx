...
export default function DataList({ listName }) {
  ...
  const handleTableList = (arrList = []) => {
    ...
    switch (listName) {
      ...
      // }, [])
      case 'sample':
        setTHead(['No.', 'Name.', 'Form', 'Chemical', 'Category', 'Organization', 'Update at', 'Tools'])
        return arrList.map((data, index) => ({...data, tbody: [index + 1, data.name, data.form?.name, data.chemical?.name, data.category?.name, data.organization?.name, convertUTCDateToLocalDate(data.updateAt), [upRight, deleteIcon]]}))
      case 'experiment':
